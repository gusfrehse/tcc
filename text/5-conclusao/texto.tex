...
Neste trabalho, explicamos a intuição por trás da síntese de imagens realísticas
e como podemos capturar essa intuição com a Equação da Renderização. Em seguida,
descrevemos como podemos utilizar o método de Monte Carlo para aproximar a
solução da Equação da Renderização, utilizando a técnica de Amostragem de
Importância. Descrevemos duas técnicas de amostragem e como combinar elas
utillizando a técnica de Amostragem de Importância Múltipla. Por fim, explicamos
como podemos formular a Equação da Renderização em uma integral de caminhos,
possibilitando a amostragem de caminhos inteiros.

Baseando-nos em um Ray Tracer pronto, implementamos um Path Tracer que utiliza
as técnicas descritas anteriormente, formulando o estimador de Monte Carlo e
...
Os resultados obtidos mostraram a complementaridade das técnicas de amostragem
...
possível implementar algoritmos mais complexos como o \emph{Bidirectional Path
Tracing}, \emph{Metropolis Light Transport} \citep{veach97}, \emph{Vertex
Connection and Merging} \citep{georgiev12}, e mais recente, ReSTIR
(\emph{Spatiotemporal Reservoir Importance Resampling}) \citep{bitterli20};
todos esses que utilizam a Amostragem de Importância Múltipla.
