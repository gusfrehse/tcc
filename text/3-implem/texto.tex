...
A implementação do \emph{Path Tracer} foi feita a partir de um \emph{Ray Tracer}
básico, implementado seguindo o texto de \citet{shirley25}. A partir dele, foram
feitas modificações principalmente para incorporar as diferentes técnicas de
amostragem e a combinação delas por meio da amostragem de importância múltipla.
Além disso, foram implementadas algumas imagens para visualizar a contribuição
de cada método e a variância em cada pixel, além de outros materiais e suas
correspondentes distribuições de probabilidade.

A saída do programa são três imagens:

\begin{description}
  \item[output.png] A imagem gerada pelo algoritmo de \emph{Path Tracing}.
  ...
  \item[mis\_viz.png] Uma imagem de visualização das técnicas de amostragem,
  onde cada pixel é colorido conforme a técnica utilizada.
\end{description}

O código foi produzido na linguagem \emph{Rust}, utilizando o compilador \texttt{rustc},
versão \texttt{1.96.0}.
...
principal que irá estimar a equação da renderização.
  ...
  \item[vec3.rs] Possui a definição da estrutura de dados \texttt{Vec3} e
    seus métodos, que serão utilizados amplamente no programa.
  ...
  \item[material.rs] Possui a implementação dos BRDFs utilizados e as funções
  para amostragem e distribuições de probabilidade de cada BRDF. Também
  utilizamos essa interface para fontes de luz e definimos uma fonte de luz
  difusa.
  ...
  \item[camera.rs] Define a estrutura \texttt{Camera} e implementa a função que
  gerará os raios iniciais para cada pixel, baseados nos parâmetros da
  estrutura.
\end{description}
...
Algoritmo~\ref{alg:mainloop}. Nele, iteramos por todos os pixels, para cada
um obtemos um raio inicial com base na câmera, computamos o integrando $N$ vezes
utilizando a função \texttt{L}, e, finalmente, normalizamos e computamos
...
\For{$i = 0$ até largura $-1$}
    ...
    \For{$k = 1$ até $N$}
        ...
        \State $(\text{radiância}, \text{mis}) \gets \Call{L}{\text{raio}, \text{mundo}, \text{luz}}$
...
Welford\footnote{Utilizamos esse algoritmo para evitar problemas de
instabilidade numérica, já que, calculando utilizando a fórmula clássica,
$\left\langle (X - \langle X \rangle)^2 \right\rangle$, acabamos fazendo a
subtração de dois números potencialmente muito próximos, aumentando
significantemente a magnitude de erros numéricos.} \citep{welford62}; e
$mis\_viz$ se refere à um valor que codifica de qual estratégia de amostragem
obtivemos a contribuição naquele pixel. 
...
A função $L$, para estimar o valor da radiância na direção dada, fará a soma
...
A ideia é criarmos um caminho base, amostrando cada ponto utilizando a
amostragem por BRDF. Porém, a cada ponto amostrado, também amostramos as fontes
de luz, criando novos caminhos completos a partir da mesma base. Assim, para o
caminho base $(x_1, \ldots, x_{k - 1})$, iremos efetivamente amostrar os caminhos
...
os casos que, ao amostrar por BRDF, obtemos um ponto em uma fonte de luz.
...
técnica de amostragem utilizada para obter o último vértice do caminho
amostrado.
...
Caso nenhum desses casos aconteça, amostramos um ponto nas fontes de luz e
...
utilizamos a estrutura \texttt{LightSampler}, que retorna um ponto na fonte de
luz e a probabilidade $p_L$ de amostragem na medida de área.
...
Para evitar caminhos muito grandes, visto que quanto maior for o caminho, menor a
...
Neste capítulo, apresentamos a implementação do integrador de \emph{Path Tracing}
desenvolvido nesse trabalho. A partir de um \emph{Ray Tracer} básico, foram
