...
amostragem e a distribuição $p$ correspondente à ela. Como
...
proporcional à $f_s$, o BSDF. Essa estratégia clássica é chamada de
\emph{Amostragem por BSDF}. Vamos chamar a distribuição de probabilidade dela
de  $p_b$.
...
forma de $f$ se aproxima à de $p_b$. Isso acontece, principalmente, quando $L$ em
sí é aproximadamente uniforme.

Alguns casos onde a amostragem por BSDF se destaca são

\begin{itemize}
  \item Cenas em céu aberto, onde o próprio céu é visto como uma fonte de luz.
    Isso acontece pois quase todas as direções tem algum valor de radiância.
    Assim o termo com mais impacto na forma de $f$ é o próprio BSDF.
  \item Materiais reflexivos, como metais e espelhos. Nesse caso, os BDSF
    desses materiais costumam ser bastante radicais, pois para haver a formação
    de uma imagem de reflexão, é necessário que a maior parte da contribuição venha
    de raios que o ângulo de inciência seja o mesmo do ângulo de reflexão, logo
\end{itemize}

Existem casos, porém, que a amostragem por BSDF não é a ideal. Em uma cena com
somente uma pequena fonte de luz, por exemplo, a chance de ``batermos'' na
fonte de luz, amostrando baseado na distribuição do BSDF, é muito pequena. Uma
cena noturna, portanto, com apenas a luz da lua e de alguns postes, precisaria
de muitas amostras para convergir para o resultado real. Olhando na definição
de $f$, a presença de apenas poucas luzes na cena se traduz no fato de que $L$
terá picos em algumas direções (direção das fontes de luz) e será próxima de
zero em outras direções. Assim, o termo $L$ `ganha' do termo $f_s$, e $f$ deixa
de ter o formato de $p_l$, consequentemente aumentando a variância de forma
significante.

Uma forma de resolver esse problema é usar outra estratégia de amostragem, como
por exemplo a \emph{Amostragem de Iluminação}, amostrada em relação a
distribuição de probabilidade $p_l$. Nessa estratégia amostramos um ponto da
superfície de uma das fontes de luz da cena e continuamos nessa direção. Assim
garantimos que estaremos em uma direção em que, caso não haja obstrução, o
valor de $L$ será significativo.

Exemplos de casos que melhor aproveitam a amostragem de iluminação são

\begin{itemize}
  \item Materiais com BSDF quase uniforme, ou seja, materiais foscos. Como BSDF
    é uniforme, o formato de $f$ é dado por $L$.
  \item Casos em que o termo de geometria seja simples, ou seja, que não haja
    muitos objetos obstruindo as fontes de luz.\footnote{No caso da amostragem
    por BSDF, por outro lado, como seguimos na direção amostrada até chegar no
    próximo ponto, nunca teremos uma obstrução, então o termo de geometria é
    capturado perfeitamente.}
\end{itemize}

Nesse trabalho, iremos utilizar essas duas estratégias de amostragem. Fazemos
essa escolha pela relativa simplicidade de implementação e pelo fato de que as
duas combinadas (ver próximas sessões) conseguem trazer um resultado
satisfatório. Porém, existem outros métodos usados para amostragem, inclusive
trata-se de uma das áreas com mais pesquisas recentemente \citep{lin22, yan25}. 
...
Com a sessão anterior, temos duas maneiras de conseguir amostras para nosso
estimador de Monte Carlo, um com a Amostragem por BSDF e outro com Amostragem
de Iluminação. Cada uma delas funciona melhor em certos casos - e é interessante notar que em casos praticamente excludentes.

TODO: explicar que somar apenas as samples com um peso fixo w = 1 ou w = 1/2 tem muita variancia.
