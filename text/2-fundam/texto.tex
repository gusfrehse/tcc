...
\[
  G(x \leftrightarrow x') = \frac{\cos{\theta_i}\cos{\theta_o}}{\left|\left|x' - x''\right|\right|}
\]

[TODO: checar formula, ver se faz mais sentido colocar aqui a versão de
pontos ou a versão direcional ou ainda a versão de caminhos]
...
invíavel evaluar $f$ em todos os elementos de $D$ para obter o valor exato de
...
\begin{align*}
  S &= \sum_{x \in D} f(x) = \sum_{x \in D} \frac{f(x)}{p(x)} \cdot p(x) = \left\langle \frac{f(X)}{p(X)} \right\rangle
\end{align*}

Onde usamos a notação $\langle Y \rangle$ para denotar a esperança de uma
variável aleatória $Y$.

Agora, para fazer a estimativa, podemos usar a Lei dos Grandes Números, que diz
que
...
Aqui a escolha ideal de $p$ é tal que $p(x) = c \cdot f(x)$ onde $c$ é
calculada para que $p$ permaneça normalizado \citep{kalos08}
...
contribuem mais para o resultado final e menos frequente amostras que pouco
afetam o resultado final, assim diminuindo o número de amostras que precisamos
para ter um resultado parecido com o original.
