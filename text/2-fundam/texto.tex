...
\[
  p_A(x) = p_\omega(\omega) \left( \frac{|\cos(\theta_i)|}{||x - x'||^2}  \right)
\]
...
\begin{equation}
  I =  \sum_{k=0}^\infty \int_{\bar{x} \in \mathcal{M}^k}
  \left(\prod_{i=1}^{k-2} f(x_{i+2} \rightarrow x_{i+1} \rightarrow x_i)
  G(x_{i+2} \leftrightarrow x_{i+1})\right) dA(x_1) \ldots dA(x_k).
\end{equation}
...
\[
  p(\bar{x}) = p_{A,1}(x_1) \ldots p_{A,k}(x_k)
\]

onde $p_{A,i}$ representa a distribuição de probabilidade na medida de área da
técnica de amostragem utilizada para obter o $i$-ésimo vértice do caminho
$\bar{x}$.
