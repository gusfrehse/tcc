...
Assim, como a luz atravessa o espaço em linha reta (pelo menos no escopo desse
trabalho), podemos seguir a direção do sensor para descobrir de qual ponto ela
está vindo e qual a intensidade. Esse ponto, que faz parte da geometria, pode
estar emitindo luz de duas maneiras. A primeira é uma emissão autônoma, que
ocorre quando o ponto é uma fonte de luz (um LED por exemplo, ou o sol). A outra
é quando existe a reflexâo de raios que bateram nesse ponto e foram para a
direção do nosso sensor.
...
infinitos raios que chegam no nesse ponto. E cada um desse novos raios poderá
...
Na década de 80, \cite{kajiya86} explicitou esse problema em uma equação
integral recursiva, baseando-se em modelos de transferência de calor já
existentes utilizados em cálculos físicos. Usaremos essa equação diversas vezes,
porém com a notação mais comumente utilizada de \citep{veach97}:
...
[IMAGEM COM x x' x'' E UM RAIO]

\section{Equação da Renderização}

A Equação \eqref{eq:render}, que é chamada de Equação da Renderização, segue o
raciocínio da seção passada: A radiância no ponto $x$ vinda do ponto $x'$ é
igual soma das das radiâncias vindas de outros pontos na geometria da cena ($L(x
\rightarrow x')$) ponderadas por um peso ($f(x \rightarrow x' \rightarrow x'')$)
e um termo de geometria ($G(x \leftrightarrow x')$).
...
porcentagem de raios de luz (ou mais formalmente a razão da radiância) que é
refletida.
...
possível contribuição de radiância vinda de $x$.
